 ...
 * specific worker's own work schedule (scripts/057-staff-module.sql), so a
 * booking can't land outside either the business's general hours or that
 * worker's individual ones. Both inputs come from generateAvailableSlots on
 * the same date/interval grid, so a plain timestamp-set intersection is
