  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          // No aspect-video here on purpose - every real usage
          // (app/(dashboard)/dashboard/analytics/page.tsx) already passes
          // an explicit h-[…px] w-full, and stacking a 16:9 aspect-ratio on
          // top of that gave the wrapper two competing height sources.
          // ResponsiveContainer's own ResizeObserver could grab either one
          // depending on which reflow won the race, which is exactly the
          // "looks zoomed in, then sometimes doesn't" flakiness reported on
          // mobile - only one source of truth for height now.
